...
class V1Dot0Dot111 extends AbstractMigration
{
    /**
     * @inheritDoc
     */
    ...
    {
        echo 'Update assets in configurations... ' . PHP_EOL;
        ...
        foreach ($pcConfigurations as $pcConfiguration) {
            ...
            if ($pcConfigurationId) {
                $pcConfiguration = $this->getEntityManager()->getEntity('PcConfiguration', $pcConfigurationId);
                if (!$pcConfiguration) {
                    continue;
                }

                $mainProductId = $pcConfiguration->get('mainProductId');
                $mainProduct = $this->getEntityManager()->getEntity('Product', $mainProductId);
                if (!$mainProduct) {
                    continue;
                }

                $service = $this->getContainer()->get('serviceFactory')->create('Product');
                $service->generateAssetsForConfiguration($pcConfiguration, $mainProduct);
            }
        }
